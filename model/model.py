...
class CrossAttentionModel(nn.Module):
    ...
    def forward(self, A, B):
        """
        :param A: (batch_size_A, seq_len_A, embed_dim)
        :param B: (batch_size_B, seq_len_B, embed_dim)
        """
...
class DeepHEx(pl.LightningModule):
    def __init__(self, lr, num_features, num_genes, num_embeddings, radius,radius_pix, bias=False):
        super(DeepHEx, self).__init__()
        ...
        self.BCAF = CrossAttentionModel(embed_dim=num_embeddings, his_dim=num_features, ge_dim=num_genes)
    ...
    def get_Multi_Fea(self, his_fea, gene_fea):
        ...
        x = self.BCAF.forward(his_fea, gene_fea)
        x = self.l1(x) + x
    ...
    def training_step(self, batch, batch_idx):
        ...
        mask = get_disk_mask(self.radius_pix)
