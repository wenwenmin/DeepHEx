...
def get_common_HVG(prefix_list, ref_prefix, n_top=1000, out_prefix="./"):
    cnts_list = [load_tsv(f"{prefix}cnts.csv") for prefix in prefix_list]

    common_genes = set(cnts_list[0].columns)
    for df in cnts_list[1:]:
        common_genes &= set(df.columns)
    common_genes = list(common_genes)

    ref_cnts = load_tsv(f"{ref_prefix}cnts.csv")

    ref_cnts = ref_cnts[common_genes]

    gene_var = ref_cnts.var().sort_values(ascending=False)

    top_genes = gene_var.index[:n_top]

    mkdir(f"{out_prefix}gene-names.txt")
    with open(f"{out_prefix}gene-names-com.txt", "w") as f:
        for g in top_genes:
            f.write(f"{g}\n")
